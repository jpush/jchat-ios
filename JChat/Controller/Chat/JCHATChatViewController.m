...
@implementation JCHATChatViewController//change name chatcontroller
- (void)viewDidLoad {
  [super viewDidLoad];
...
- (void)prepareImageMessage:(UIImage *)img {
  ...
  NSLog(@"huangmin   imagemessage   %p",message);
  [self addmessageShowTimeData:message.timestamp];
  [model setChatModelWith:message conversationType:_conversation];
  [_imgDataArr addObject:model];
  model.photoIndex = [_imgDataArr count] - 1;
...
heightForRowAtIndexPath:(NSIndexPath *)indexPath {
  ...
  if (model.message.contentType == kJMSGContentTypeEventNotification || model.isTime == YES) {
    return 31;
  }
