...
//  all photoAlbumdel
  ...
  HMAlbumModel *albumModel = [HMAlbumModel new];
...
//  smartAlbums
  ...
  for (PHAssetCollection *enumCollection in smartAlbums) {
    ...
    HMAlbumModel *model = [HMAlbumModel new];
...
- (void)pushToSelectPhotoVCWithIndex:(NSInteger)index {
  HMPhotoSelectViewController *selectPhotoVC = [[HMPhotoSelectViewController alloc] init];
  HMAlbumModel *model = _albumArr[index];
