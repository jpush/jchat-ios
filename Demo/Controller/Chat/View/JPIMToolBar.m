...
- (void)drawRect:(CGRect)rect {
    ...
    self.recordAnimationView=[[RecordAnimationView alloc]initWithFrame:CGRectMake((kApplicationWidth-140)/2, (kScreenHeight -kNavigationBarHeight - kTabBarHeight - 140)/2, 140, 140)];
...
- (void)recordingFinishedWithFileName:(NSString *)filePath time:(NSTimeInterval)interval
{
    ...
    if (interval < 0.50) {
        [JPIMFileManager deleteFile:filePath];
        return;
    }
    dispatch_async(dispatch_get_main_queue(), ^{
...
+ (CGFloat)maxHeight {
    return ([JPIMToolBar maxLines] + 1.0f) * [JPIMToolBar textViewLineHeight];
}
